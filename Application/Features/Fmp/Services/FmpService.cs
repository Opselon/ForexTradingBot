...
namespace Application.Features.Fmp.Services
{
    public class FmpService : IFmpService
    {
        ...
        {
            var quotesResult = await _apiClient.GetFullCryptoQuoteListAsync(cancellationToken);
            if (!quotesResult.Succeeded || quotesResult.Data == null) { return Result<List<FmpQuoteDto>>.Failure(quotesResult.Errors); }
            var topCryptos = quotesResult.Data.Where(q => q.MarketCap.HasValue && q.MarketCap > 0 && q.Name != null).OrderByDescending(q => q.MarketCap).Take(count).ToList();
